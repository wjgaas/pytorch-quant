...
def eval_model(model, ds, n_sample=None, ngpu=1, model_raw=None):
    import tqdm
    import torch
    from torch import nn
    ...
    with torch.no_grad():
        for idx, (input, target) in enumerate(tqdm.tqdm(ds, total=n_sample)):
            ...
            output = model(input)
            #print('final_output')
            #print(output.view(-1).sort(descending=True)[0])
            if model_raw != None:
                output_raw = model_raw(input)
                print('final_output_raw')
                print(output_raw.view(-1).sort(descending=True)[0])               
